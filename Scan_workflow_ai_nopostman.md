...
# Step 8: Get Scan Results (Final Step)
...
If not, stop and instruct them to complete the previous required step
first.
...
Optional query parameters: - `severity` - `state` - `status` -
`offset` - `limit` - `sort` - `exclude-result-types`
...
Required: - `scan-id={scan_id}`

Optional: - `severity` - `state` - `status` - `offset` - `limit` -
`sort` - `exclude-result-types`

------------------------------------------------------------------------

## Provide Default Example (curl)

``` bash
curl -X GET "{base_url}/api/results?scan-id={scan_id}&limit=20" \
...
1.  Look for the required fields:

    -   `totalCount`
    -   `results`

2.  Paste the full JSON response.

3.  The assistant will:

    -   extract `totalCount`
    -   confirm how many findings were returned

4.  Provide a very brief summary:

    -   number of findings
    -   highest severity present (if any)

5.  Do NOT provide deep analysis or remediation guidance.

------------------------------------------------------------------------

## 🏁 Workflow Completion (Endgame Rule)

After processing the scan results response, the assistant MUST:

1.  Output the following message:

🎉 Congratulations --- you have successfully completed the full
Checkmarx introductory workflow.
...
4.  End the response immediately after this message.
